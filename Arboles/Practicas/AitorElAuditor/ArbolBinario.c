...
void eliminarElementoArbol(tArbol* arbol, tElemento elemento) {
    ...
    if (*arbol != NULL) {
        ...
        if (esIgual((*arbol)->elemento, elemento)) {
            ...
            eliminarElementoArbol(&(*arbol)->derecha, elemento);
        } else {
            eliminarElementoArbol(&(*arbol)->izquierda, elemento);
        }
    }
}
tLista listarInferiores(tArbol arbol, int limite) {
    tLista lista, listaIzq, listaDer;
    crearListaVacia(&lista);

    if (!esArbolVacio(arbol)) {
        listaIzq = listarInferiores(arbol->izquierda, limite);

        if (arbol->elemento.sueldo < limite) {
            insertarElementoInicio(&listaIzq, arbol->elemento);
            listaDer = listarInferiores(arbol->derecha, limite);
